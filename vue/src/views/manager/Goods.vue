<template>
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      ...
      <el-table tooltip-effect="dark myEffect" :data="data.tableData" stripe>
        ...
        <el-table-column label="推荐">
          <template #default="scope">
            <el-switch @change="updateRecommend(scope.row)"
    ...
    <el-dialog title="商品信息" width="50%" v-model="data.formVisible" :close-on-click-modal="false" destroy-on-close>
      <el-form ref="formRef" :model="data.form" :rules="data.rules" label-width="80px" style="padding-right: 30px;padding-top: 20px">
        ...
        <el-form-item prop="recommend" label="推荐状态">
...
const load = () => {
  request.get('/goods/selectPage', {
    ...
    data.tableData.forEach(item => {
      item.isRecommend = item.recommend === '是'
    })
  })
}
load()
...
const updateRecommend = (row) => {
  data.form.id = row.id
  data.form.recommend = row.isRecommend ? '是' : '否'
  update()
}
